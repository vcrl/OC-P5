...
class Database_Executions:
    """
        ...
        *Methods:
        ...
        """
    def __init__(self):
        self.db = mysql.connect(host=DB_HOST, user=DB_USER, password=DB_PSWD, database=DB_DB) # config
    ...
    def create_from_api(self):
        ...
        for category in CATEGORIES.values():
            ...
            for product in response['products']:
                ...
                self.printProgressBar(percentage_index, total, prefix=f'Chargement de: {category}')
                
                check = False
                for element in ['brands', 'ingredients_text_debug', 'image_front_url']:
                    if product[element]:
                        check = True
                if check:
                    try:
                        ...
                        cursor.execute(f"DELETE FROM product WHERE name = '' OR description = '' ")
                        self.db.commit()
                    except KeyError:
                        pass
